...
class CodeTool {
   ...
   *
   * @param {object} tool - tool constricting options
   * @param {CodeData} tool.data — previously saved plugin code
  ...
  constructor({ data, config, api, readOnly }) {
    ...
    this.languageList = config.languages ?? {
      'php': 'PHP',
      'js': 'JavaScript',
      'css': 'CSS'
    }
    this.languageList = Object.assign({
      '': this.api.i18n.t('Choose language')
    }, this.languageList);
  ...
  drawView() {
    ...
    // Add empty value.
    Object.entries(this.languageList).forEach(([code, label]) => {
   ...
   *
   * @param {HTMLDivElement} codeWrapper - CodeTool's wrapper, containing
   *   textarea with code
